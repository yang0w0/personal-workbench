...
> **内置分类只增不减，读取时自动补齐**：`M1`/`M3` 读索引时会把默认表里缺失的内置分类按 `key`（或同名 `folder`）补进去，所以**新增一个内置分类不需要迁移脚本**——老的 `catalog.json` 下次读取就会多出这一项。反过来，已存在同名 `key` 或同名 `folder` 时不会重复添加。
>
> **`key` 与 `kind` 不是一一对应的**：「文件夹」和「文档」共用 `file` 这一种 `kind`（`KINDS` 只有 `script`/`link`/`app`/`file` 四个值）。`kind` 决定编辑器显示哪组字段、点击时怎么打开；`key` 只用来标识分类本身。所以「多一个分类」通常**不需要**动 `kind` 枚举。
>
> 「文件夹」的语义：条目只存**绝对路径**（放在 `target`），不写 `sourcePath`、也不在 `data/` 下建实体文件；点击后由系统资源管理器打开该目录，用于「一键跳到本地某个文件夹」。正因为它没有 `sourcePath`，扫描的失效清理不会误删它，而且它是唯一一个**条目路径在 `data/` 之外**的内置分类。
>
> 两端的默认分类现在是同一套（含 `应用`/`文档`/`文件夹`）。除此之外，`M1` 与 `M3` 在读取时都会**扫描 `data/` 下已存在的文件夹并补登为分类**，所以用户自建的 `密码库`/`备份` 等目录也会出现在索引里（它们只是空分类，不带计数徽标）。
