...
这是「`symbol` 是个字符型占位符号」（`▤`、`◆`、`↗`）的时代留下的限制。而前端现在的图标选择器会把**图标名**当 `symbol` 发出去（例如 `lock` / `archive` / `clock`），后端会截成 `lo` / `ar` / `cl`，前端认不出，于是回落到「按名称关键词猜 → 按 `kind` 取默认图标」（解析顺序见 [app-ui.md](interfaces/app-ui.md#55-内置图标集)）。
