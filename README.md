...
- `workbench.html`：单文件桌面界面，HTML、CSS 和 JavaScript 全部内嵌，不加载任何相对路径的静态资源。
...
已在此电脑成功生成根目录的 `个人工作台.exe`。以后修改客户端代码后，双击 `build-client.cmd` 即可重新构建并覆盖它。完整规则见 [客户端说明](docs/CLIENT.md)。
